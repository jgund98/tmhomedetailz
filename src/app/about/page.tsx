          ...
          <div className="grid gap-14 lg:grid-cols-[1fr_1.1fr] lg:gap-20">
              ...
              <div className="lg:sticky lg:top-28">
                ...
                <Reveal delay={0.1}>
                  <div className="mt-6 grid grid-cols-2 gap-4">
                    {STATS.map((s) => (
                      <div key={s.label} className="rounded-2xl border border-hydro/15 bg-trench p-5">
                        <p className="display text-2xl text-hydro">
                          {s.value.toLocaleString()}
                          {s.suffix}
                        </p>
                        <p className="mt-1.5 text-xs leading-snug text-mist-dim">{s.label}</p>
                      </div>
                    ))}
                  </div>
                </Reveal>
            ...
            <div>
              ...
              <div className="relative mt-14">
                ...
                <span className="absolute bottom-2 left-[7px] top-2 w-0.5 bg-gradient-to-b from-hydro via-brand to-hydro/20" aria-hidden="true" />
                <div className="flex flex-col gap-12">
                  {TIMELINE.map((t, i) => (
                    <Reveal key={t.title} delay={i * 0.05}>
                      <div className="relative pl-10">
                        <span className="absolute left-0 top-1.5 grid h-4 w-4 place-items-center" aria-hidden="true">
                          <span className="h-4 w-4 rounded-full border-2 border-hydro bg-abyss" />
                          <span className="absolute h-1.5 w-1.5 rounded-full bg-hydro" />
                        </span>
                        <p className="label text-hydro">{t.year}</p>
                        <h3 className="display mt-2 text-2xl md:text-3xl">{t.title}</h3>
                        <p className="mt-3 max-w-lg text-base leading-relaxed text-mist">{t.body}</p>
                      </div>
                    </Reveal>
                  ))}
                </div>
              ...
              <Reveal delay={0.1}>
                <div className="mt-16 rounded-2xl border border-hydro/15 bg-trench p-8 md:p-10">
                  <p className="label mb-4 text-hydro">Why &ldquo;Detailz&rdquo;?</p>
